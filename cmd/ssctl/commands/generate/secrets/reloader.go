...
func NewReloaderSubCommand(parentCommand *cobra.Command) {
	generateCmd := &cmdgenerate.GenerateSubCommand{
		...
		GenerateFunc:          GenerateReloader,
		UseCollectionManifest: true,
		CollectionKind:        "SupportServices",
		UseWorkloadManifest:   true,
		WorkloadKind:          "Reloader",
	}

	generateCmd.Setup()
}

// GenerateReloader runs the logic to generate child resources for a
// Reloader workload.
func GenerateReloader(g *cmdgenerate.GenerateSubCommand) error {
	...
	generateFuncMap := map[string]generateFunc{
		"v1alpha1": v1alpha1reloader.GenerateForCLI,
