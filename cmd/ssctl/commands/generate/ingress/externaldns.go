...
func NewExternalDNSSubCommand(parentCommand *cobra.Command) {
	generateCmd := &cmdgenerate.GenerateSubCommand{
		Name:                  "external-dns",
		Description:           "Manage external-dns installation for ingress support services",
		SubCommandOf:          parentCommand,
		GenerateFunc:          GenerateExternalDNS,
		UseCollectionManifest: true,
		CollectionKind:        "SupportServices",
		UseWorkloadManifest:   true,
		WorkloadKind:          "ExternalDNS",
	}

	generateCmd.Setup()
}

// GenerateExternalDNS runs the logic to generate child resources for a
// ExternalDNS workload.
func GenerateExternalDNS(g *cmdgenerate.GenerateSubCommand) error {
	...
	generateFuncMap := map[string]generateFunc{
		"v1alpha1": v1alpha1externaldns.GenerateForCLI,
