...
func NewSecretsComponentSubCommand(parentCommand *cobra.Command) {
	generateCmd := &cmdgenerate.GenerateSubCommand{
		...
		GenerateFunc:          GenerateSecretsComponent,
		UseCollectionManifest: true,
		CollectionKind:        "SupportServices",
		UseWorkloadManifest:   true,
		WorkloadKind:          "SecretsComponent",
	}

	generateCmd.Setup()
}

// GenerateSecretsComponent runs the logic to generate child resources for a
// SecretsComponent workload.
func GenerateSecretsComponent(g *cmdgenerate.GenerateSubCommand) error {
	...
	generateFuncMap := map[string]generateFunc{
		"v1alpha1": v1alpha1secretscomponent.GenerateForCLI,
