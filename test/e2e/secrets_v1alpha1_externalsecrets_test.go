...
//
// secretsv1alpha1ExternalSecrets tests
//
func secretsv1alpha1ExternalSecretsChildrenFuncs(tester *E2ETest) error {
	// TODO: need to run r.GetResources(request) on the reconciler to get the mutated resources
	if len(externalsecrets.CreateFuncs) == 0 {
		return nil
	}

	workload, collection, err := externalsecrets.ConvertWorkload(tester.workload, tester.collectionTester.workload)
	if err != nil {
		return fmt.Errorf("error in workload conversion; %w", err)
	}

	resourceObjects, err := externalsecrets.Generate(*workload, *collection, nil, nil)
	if err != nil {
...
func secretsv1alpha1ExternalSecretsNewHarness(namespace string) *E2ETest {
	return &E2ETest{
		namespace:          namespace,
		unstructured:       &unstructured.Unstructured{},
		workload:           &secretsv1alpha1.ExternalSecrets{},
		sampleManifestFile: "../../config/samples/secrets_v1alpha1_externalsecrets.yaml",
		getChildrenFunc:    secretsv1alpha1ExternalSecretsChildrenFuncs,
		logSyntax:          "controllers.secrets.ExternalSecrets",
		collectionTester:   orchestrationv1alpha1SupportServicesNewHarness(""),
	}
}

func (tester *E2ETest) secretsv1alpha1ExternalSecretsTest(testSuite *E2EComponentTestSuite) {
...
func (testSuite *E2EComponentTestSuite) Test_secretsv1alpha1ExternalSecrets() {
	tester := secretsv1alpha1ExternalSecretsNewHarness("")
	tester.secretsv1alpha1ExternalSecretsTest(testSuite)
}
