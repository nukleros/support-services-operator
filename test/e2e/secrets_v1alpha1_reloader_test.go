...
//
// secretsv1alpha1Reloader tests
//
func secretsv1alpha1ReloaderChildrenFuncs(tester *E2ETest) error {
	// TODO: need to run r.GetResources(request) on the reconciler to get the mutated resources
	if len(reloader.CreateFuncs) == 0 {
		return nil
	}

	workload, collection, err := reloader.ConvertWorkload(tester.workload, tester.collectionTester.workload)
	if err != nil {
		return fmt.Errorf("error in workload conversion; %w", err)
	}

	resourceObjects, err := reloader.Generate(*workload, *collection, nil, nil)
	if err != nil {
...
func secretsv1alpha1ReloaderNewHarness(namespace string) *E2ETest {
	return &E2ETest{
		namespace:          namespace,
		unstructured:       &unstructured.Unstructured{},
		workload:           &secretsv1alpha1.Reloader{},
		sampleManifestFile: "../../config/samples/secrets_v1alpha1_reloader.yaml",
		getChildrenFunc:    secretsv1alpha1ReloaderChildrenFuncs,
		logSyntax:          "controllers.secrets.Reloader",
		collectionTester:   orchestrationv1alpha1SupportServicesNewHarness(""),
	}
}

func (tester *E2ETest) secretsv1alpha1ReloaderTest(testSuite *E2EComponentTestSuite) {
...
func (testSuite *E2EComponentTestSuite) Test_secretsv1alpha1Reloader() {
	tester := secretsv1alpha1ReloaderNewHarness("")
	tester.secretsv1alpha1ReloaderTest(testSuite)
}
