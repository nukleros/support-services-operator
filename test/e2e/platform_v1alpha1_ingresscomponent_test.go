...
func platformv1alpha1IngressComponentChildrenFuncs(tester *E2ETest) error {
	...
	workload, collection, err := ingresscomponent.ConvertWorkload(tester.workload, tester.collectionTester.workload)
	if err != nil {
		return fmt.Errorf("error in workload conversion; %w", err)
	}

	resourceObjects, err := ingresscomponent.Generate(*workload, *collection, nil, nil)
	if err != nil {
...
func platformv1alpha1IngressComponentNewHarness(namespace string) *E2ETest {
	return &E2ETest{
		namespace:          namespace,
		unstructured:       &unstructured.Unstructured{},
		workload:           &platformv1alpha1.IngressComponent{},
		sampleManifestFile: "../../config/samples/platform_v1alpha1_ingresscomponent.yaml",
		getChildrenFunc:    platformv1alpha1IngressComponentChildrenFuncs,
		logSyntax:          "controllers.platform.IngressComponent",
		collectionTester:   setupv1alpha1SupportServicesNewHarness(""),
	}
}

func (tester *E2ETest) platformv1alpha1IngressComponentTest(testSuite *E2EComponentTestSuite) {
...
func (testSuite *E2EComponentTestSuite) Test_platformv1alpha1IngressComponent() {
	tester := platformv1alpha1IngressComponentNewHarness("")
	tester.platformv1alpha1IngressComponentTest(testSuite)
}
