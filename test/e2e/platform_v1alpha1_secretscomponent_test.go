...
func platformv1alpha1SecretsComponentChildrenFuncs(tester *E2ETest) error {
	...
	workload, collection, err := secretscomponent.ConvertWorkload(tester.workload, tester.collectionTester.workload)
	if err != nil {
		return fmt.Errorf("error in workload conversion; %w", err)
	}

	resourceObjects, err := secretscomponent.Generate(*workload, *collection, nil, nil)
	if err != nil {
...
func platformv1alpha1SecretsComponentNewHarness(namespace string) *E2ETest {
	return &E2ETest{
		namespace:          namespace,
		unstructured:       &unstructured.Unstructured{},
		workload:           &platformv1alpha1.SecretsComponent{},
		sampleManifestFile: "../../config/samples/platform_v1alpha1_secretscomponent.yaml",
		getChildrenFunc:    platformv1alpha1SecretsComponentChildrenFuncs,
		logSyntax:          "controllers.platform.SecretsComponent",
		collectionTester:   setupv1alpha1SupportServicesNewHarness(""),
	}
}

func (tester *E2ETest) platformv1alpha1SecretsComponentTest(testSuite *E2EComponentTestSuite) {
...
func (testSuite *E2EComponentTestSuite) Test_platformv1alpha1SecretsComponent() {
	tester := platformv1alpha1SecretsComponentNewHarness("")
	tester.platformv1alpha1SecretsComponentTest(testSuite)
}
