...
func certificatesv1alpha1CertManagerChildrenFuncs(tester *E2ETest) error {
	...
	workload, collection, err := certmanager.ConvertWorkload(tester.workload, tester.collectionTester.workload)
	if err != nil {
		return fmt.Errorf("error in workload conversion; %w", err)
	}

	resourceObjects, err := certmanager.Generate(*workload, *collection, nil, nil)
	if err != nil {
